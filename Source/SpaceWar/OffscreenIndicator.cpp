...
void UOffscreenIndicator::FindScreenEdgeLocationForWorldLocation(UObject* WorldContextObject, const FVector& InLocation,
                                                                 ...
                                                                 float& OutRotationAngleDegrees, bool& bIsOnScreen)
{
	...
	FVector Offset = (InLocation - PlayerPawn->GetActorLocation()).GetSafeNormal();
	...
	if (bLocationIsBehindCamera)
	{
		...
		FVector DiffVector = InLocation - PlayerPawn->GetActorLocation();
		FVector Inverted = DiffVector * -1.f;
		FVector NewInLocation = PlayerPawn->GetActorLocation() * Inverted;

		NewInLocation.Z -= 10000;
		//NewInLocation.Z = 0;
